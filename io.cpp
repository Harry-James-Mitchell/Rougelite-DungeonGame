...
uint32_t io_wear_eq(dungeon_t *d)
{
  ...
  while (1) {
    ...
    if (d->PC->in[key - '0'] && d->PC->in[key - '0']->is_equipable()) {
        d->PC->carry += d->PC->in[key-'0']->get_weight();
        d->PC->defence += d->PC->in[key-'0']->get_defence();
        d->PC->wear_in(key-'0');
        io_display(d);
      return 0;
    }
...
//      if (d->PC->in[key - '0']) {
//
//          d->PC->money += d->PC->in[key-'0']->get_value()/2;
...
uint32_t io_remove_eq(dungeon_t *d)
{
  ...
  while (1) {
    ...
    if (d->PC->eq[key-'a'] && d->PC->in[key-'a']->is_removable() && d->PC->has_open_inventory_slot()) {
        d->PC->carry -= d->PC->eq[key-'a']->get_weight();
        d->PC->defence -= d->PC->eq[key-'a']->get_defence();
        d->PC->remove_eq(key - 'a');
        io_display(d);
      return 0;
    }
...
void io_show_pc_stuff(dungeon_t *d){
    clear();
    mvprintw(0,0,"Name: %s",d->PC->name);
  mvprintw(1,0,"HP: %d/%d  Next heal in %d turns",d->PC->hp,d->PC->mhp,d->PC->next_heal);
    mvprintw(2,0,"Money: %d",d->PC->money);
    mvprintw(3,0,"Carry: %d/25",d->PC->carry);
    mvprintw(4,0,"Speed: %d",d->PC->speed);
    mvprintw(5,0,"Defence: %d",d->PC->defence);


    mvprintw(15,0,"Kills: %d",d->PC->kills[kill_direct]);
    mvprintw(16,0,"Floor: %d",d->floor);
...
//      break;
    ...
    case 'I':
      ...
      break;
//    case '$':
//        io_queue_message("Money: %d",d->PC->money);
//            io_display(d);
//        fail_code = 1;
//        break;
    case '!':
...
void display_shop(dungeon_t *d){
  ...
  clear();
  mvprintw(0,0,"Welcome to my shop %s",d->PC->name);
  mvprintw(1,0,"%s","What would you like to buy?");
  mvprintw(3,0,"%s%d","Gold: ",d->PC->money);
  for (i=0;i<3;i++){
    mvaddch(i+5,0,i+'0');
    mvaddch(i+5,1,')');
      vals[i] = rand_range(0,v.size()-1);
      os[i] = new object(v[vals[i]], p, NULL);
      mvprintw(i+5,2,"%s   G:%d",os[i]->get_name(),os[i]->get_value());
  }
  while(flag){
    switch(i=getch()){
      case '0':
      case '1':
      case '2':
        if(d->PC->money < os[i-'0']->get_value()){
          mvprintw(8,0,"%s","You don't have enough money to buy that");
        } else if(!d->PC->has_open_inventory_slot()){
          mvprintw(8,0,"%s","You don't have any room in your inventory for that");
        } else {
          d->PC->money -= os[i-'0']->get_value();
          d->PC->in[d->PC->get_first_open_inventory_slot()] = os[i-'0'];
          mvprintw(8,0,"%s%s","You buy the " ,os[i-'0']->get_name());
        }
        flag = 0;
        break;
    case 27:
  ...
  refresh();
  mvprintw(17,0,"%s","press any button to continue.");
  while(!getch()){}
  clear();
  io_display(d);
}
