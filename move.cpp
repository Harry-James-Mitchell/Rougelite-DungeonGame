    ...
    if(atk == d->PC && def->symbol == '$'){
        ...
        if (character_is_alive(def)) {
            if (atk != d->PC) {
                ...
                if(damage < (uint32_t)d->PC->defence){damage = 1;}
                else {damage -= d->PC->defence;}
...
void move_character(dungeon_t *d, character *c, pair_t next)
{
  ...
  if (charpair(next) && ((next[dim_y] != c->position[dim_y]) || (next[dim_x] != c->position[dim_x]))) {
    if ((charpair(next) == d->PC) || c == d->PC) {
      ...
      for (s = rand() % 9, found_cell = i = 0; i < 9 && !found_cell; i++) {
        ...
        if (((npc *) charpair(next))->characteristics & NPC_PASS_WALL) {
          if (!charpair(displacement) || (charpair(displacement) == c)) {
            found_cell = 1;
          }
        } else {
          if ((!charpair(displacement) && (mappair(displacement) >= ter_floor)) || (charpair(displacement) == c)) {
            found_cell = 1;
          }
        }
      }
...
void do_moves(dungeon_t *d)
{
  ...
  while (pc_is_alive(d) &&
    ...
    move_character(d, c, next);
      ...
      if(!character_get_speed(c)){
          heap_insert(&d->events, update_event(d, e, 999999999));
      } else {
...
void heal_character(character *c){
    c->next_heal--;
    if(!c->next_heal){
        c->hp += c->mhp/10;
        c->next_heal = 5;
